...
##### Family-Wise Error Rate: $\mathrm { FWER } = P [ 1$ or more type $|$ errors $] = P [ V \geq 1 ]$
...
adjust p-values: $P _ { \mathrm { adj } , ( i ) } = \min \left\{ ( m - i + 1 ) \cdot P _ { ( i ) } , 1 \right\} ;$ if value below $P _ { \mathrm { adj } , ( i - 1 ) } ,$ replace it by $P _ { \mathrm { adj } , ( i - 1 ) }$  
...
##### False discovery rate: $\mathrm { FDR } = E \left[ \frac { \mathrm { FP } } { \mathrm { FP } + \mathrm { TP } } \right] = E \left[ \frac { V } { R } \right]$  
...
order p-values: $P _ { ( 1 ) } \leq P _ { ( 2 ) } \leq P _ { ( 3 ) } \leq \ldots \leq P _ { ( m ) }$  
adjust p-values to get q-values: $Q _ { ( i ) } = \min \left\{ \frac { m } { i } \cdot P _ { ( i ) } , 1 \right\} ;$ if value below $Q _ { ( i - 1 ) , }$ replace it by $Q _ { ( i - 1 ) }$   
...
$Y _ { i }$ : response variable  
$x _ { i }$ : explanatory variable  
$E _ { i }$ : error or noise variables  
Residuals: $R _ { i } = Y _ { i } - \hat { \beta } _ { 0 } - \hat { \beta } _ { 1 } x _ { i }$  
Residual Sum of Squares: RSS $= \sum _ { i = 1 } ^ { n } R _ { i } ^ { 2 }$  
Minimizers $\hat { \beta } _ { 0 }$ and $\hat { \beta } _ { 1 }$ of RSS are unbiased estimators for the true coefficients $\beta _ { 0 }$ and $\beta _ { 1 }$  
$R _ { i } = Y _ { i } - \hat { \beta } _ { 0 } - \hat { \beta } _ { 1 } x _ { i }$  
...
$\hat { \beta } _ { 1 } = \frac { s _ { x y } } { s _ { x } ^ { 2 } }$  
$s _ { x y } = \frac { 1 } { n - 1 } \sum _ { i = 1 } ^ { n } \left( x _ { i } - \overline { x } \right) \left( y _ { i } - \overline { y } \right)$  
