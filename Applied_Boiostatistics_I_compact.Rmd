...
geometry: "left=0.7cm,right=0.7cm,top=0cm,bottom=0.7cm"
output:
...
General: $A _ { 1 } , A _ { 2 } , \dots , A _ { n }$ events:   
...
$A \subset \Omega : P [ A ] = \sum _ { i \omega _ { i } \in A } P \left[ \left\{ \omega _ { i } \right\} \right]$  
$P [ \Omega ] = \sum _ { i \geq 1 } P \left[ \left\{ \omega _ { i } \right\} \right]$  
$\Omega$ is finite often: $P \left[ \left\{ \omega _ { i } \right\} \right] = 1 / | \Omega |$

##### Independence if: $P [ A \cap B ] = P [ A ] \cdot P [ B ]$

##### Conditional probability: $P [ A | B ] = \frac { P [ A \cap B ] } { P [ B ] }$ , given $P [ B ] > 0$ 

##### Law of total probability
for disjoint $B _ { 1 } , B _ { 2 } , \ldots , B _ { k }$ and $B _ { 1 } , B _ { 2 } , \ldots , B _ { k } = \Omega$:  
$P [ A ] = \sum _ { i = 1 } ^ { k } P \left[ A \cap B _ { i } \right] = \sum _ { i = 1 } ^ { k } P [ A | B _ { i } ] P \left[ B _ { i } \right]$

##### Bayes' theorem:
$P [ B | A ] = \frac { P [ A \cap B ] } { P [ A ] } = \frac { P [ A | B ] P [ B ] } { P [ A ] }$, given $P [ A ] > 0$ and $P [ B ] > 0$  
Total probability: $P \left[ B _ { i } | A \right] = \frac { P \left[ A \cap B _ { i } \right] } { P [ A ] } = \frac { P [ A | B _ { i } ] P \left[ B _ { i } \right] } { \sum _ { j = 1 } ^ { k } P [ A | B _ { j } ] P \left[ B _ { j } \right] }$

##### Cumulative distribution function (CDF) of a random variable $X$: $F _ { X } ( x ) : = P [ X \leq x ]$
...
##### Expectation value: $E [ X ] : = \sum _ { k } x _ { k } p \left( x _ { k } \right)$  
continuous: $E [ X ] = \int _ { - \infty } ^ { \infty } x f ( x ) \mathrm { d } x$

##### Variance: $\operatorname { Var } ( X ) : = \sum _ { k } \left( x _ { k } - E [ X ] \right) ^ { 2 } p \left( x _ { k } \right)$ = $\operatorname { SD ^ { 2 } } ( X )$ : Standard Deviation $^ { 2 }$
continuous: $\operatorname { Var } ( X ) = \int _ { - \infty } ^ { \infty } ( x - E [ X ] ) ^ { 2 } f ( x ) d x$

##### Bernoulli distribution $X \in \{ 0,1 \}$ $X \sim$ Bernoulli( $\pi )$ , $\pi : = P [ X = 1 ]$  
...
* Approx.: ($n \pi > 5$ & $n ( 1 - \pi ) > 5$) ? $X \approx \mathcal { N } ( n \pi , n \pi ( 1 - \pi ) )$ (only CDF) ; ($n \geq 100$ & $\pi \leq 0.01$ & $n \pi \leq 20$) ? $X \approx \operatorname { Pois } ( n \pi )$  
...
* $X \sim \operatorname { Pois } \left( \lambda _ { 1 } \right)$ , $Y \sim \operatorname { Pois } \left( \lambda _ { 2 } \right)$ independent: $X + Y \sim \operatorname { Pois } \left( \lambda _ { 1 } + \lambda _ { 2 } \right)$  
...
* Haldane's model: (position of the first crossover) $X \sim \operatorname { Exp } ( 1 )$ 
...
#### Descriptive Statistics: Model: $\quad X _ { 1 } , X _ { 2 } , \ldots , X _ { n } \stackrel { i.i.d. } { \sim } F _ { X } ( \cdot )$ , Sample: $x _ { 1 } , x _ { 2 } , \dots , x _ { n }$
Sample MEAN: $\overline { x } = \frac { x _ { 1 } + \ldots + x _ { n } } { n } = \frac { 1 } { n } \sum _ { i = 1 } ^ { n } x _ { i } = S _ { n } / n \rightarrow \mu = E [ X ]$ if $n \rightarrow \infty$ (consistent/unbiased estimator for the true mean)
...
### Histogram: bins $h _ { k } : = \# \{ i | x _ { i } \in \left( c _ { k - 1 } , c _ { k } \right] \}$ density $\frac { h _ { k } } { n \left( c _ { k } - c _ { k - 1 } \right) }$ , Box plot IQR ("interquartile range"): $q _ { 0.75 } - q _ { 0.25 }$
...
Empirical CORrelation: $r = \frac { s _ { x y } } { s _ { x } s _ { y } } \in [ - 1,1 ]$ , empirical COVariance: $s _ { x y } = \frac { 1 } { n - 1 } \sum _ { i = 1 } ^ { n } \left( x _ { i } - \overline { x } \right) \left( y _ { i } - \overline { y } \right)$  
Linear dependence between 2 samples $\left\{ x _ { i } \right\}$ and $\left\{ y _ { i } \right\}$
* $r = + 1$ if $y _ { i } = a + b x _ { i }$ for some $b > 0$
* $r = - 1$ if $y _ { i } = a + b x _ { i }$ for some $b < 0$

##### Standard error of the mean (SEM) = Standard Deviation of the mean
...
##### Law of large numbers: $X$ random variable and $E [ X ] = \mu$, and $X _ { 1 } , X _ { 2 } , \ldots , X _ { n }$ i.i.d. from $X .$ => $\overline { X } _ { n } \rightarrow \mu$ as $n \rightarrow \infty$
ECDF $\rightarrow$ CDF as $n \rightarrow \infty$

##### Central Limit Theorem
Let $X$ be random variable with expectation value $\mu$ and variance $\sigma ^ { 2 } ,$ and
$X _ { 1 } , X _ { 2 } , \ldots , X _ { n }$ i.i.d. from $X$ .  
...
$X \sim \mathcal { N } \left( \mu , \sigma ^ { 2 } \right)$ and $Y = a + b X$: $Y \sim \mathcal { N } \left( a + b \mu , b ^ { 2 } \sigma ^ { 2 } \right)$  
...
Maxiumum likelihood estimator (MLE) for $\theta : \hat { \theta } =$ value of $\theta$ for which $\ell$ attains its maximum; suitable for large samples
...
Maximum a posteriori (MAP) estimator: $\hat { \theta }$ that maximizes the $posterior\ P [ \Theta = \theta | X = x ]$ , suitable for small samples
...
In large sample limit, $n \rightarrow \infty :$ MAP (Bayesian) estimate converges to ML (frequentist) estimate
...
6) Test decision: reject $H _ { 0 }$ if $X \in K$ otherwise fail to reject.  

|       |             | Decision           |                   |     
|-------|-------------|--------------------|-------------------|
|       |             | $H _ { 0 }$        | $H _ { A  }$      |
| Truth | $H _ { 0 }$ | true negative (TN) | type I error (FP) |
|       | $H _ { A }$ | type II error (FN) | true positive (TP)|  

* Significance level $\alpha : \alpha = P [ FP | H _ { 0 } ]$ 
* Power $1 - \beta : \beta =  P [ FN | H _ { A } ]$ ; higher Significance(smaller $\alpha$) => lower Power ; bigger $\alpha$ or larger sample => higher Power  
...
$H _ { 0 } : \mu = \mu _ { 0 } = 0$ ; $H _ { \mathrm { A } } : \mu \neq \mu _ { 0 }$  
Test statistic: $T = \frac { \sqrt { n } \left( \overline { X } - \mu _ { 0 } \right) } { s _ { x } }$  with T distribution | $H _ { 0 }$ with (n - 1) degrees of freedom
...
##### T-Test
$X _ { 1 } , \ldots , X _ { n } \stackrel { \mathrm { i } . \mathrm { i.d } } \cdot \mathcal { N } \left( \mu _ { X } , \sigma ^ { 2 } \right)$ , $Y _ { 1 } , \ldots , Y _ { m } \stackrel { \mathrm { i } . \mathrm { i.d } } { \sim } \mathcal { N } \left( \mu _ { Y } , \sigma ^ { 2 } \right)$ , $H _ { 0 } : \mu _ { X } = \mu _ { Y }$ , $H _ { \mathrm { A } } : \mu _ { X } \neq \mu _ { Y }$  
Test statistic: $T = \frac { \overline { X } - \overline { Y } } { s _ { \text { pool } } \sqrt { 1 / n + 1 / m } }$  , $s _ { \mathrm { pool } } ^ { 2 } = \frac { 1 } { n + m - 2 } \left( ( n - 1 ) s _ { x } ^ { 2 } + ( m - 1 ) s _ { y } ^ { 2 } \right)$  , $T$ distribution under $H _ { 0 } : T \sim t _ { n + m - 2 }$
...
##### Effect size Two samples: Experimental group $\left\{ X _ { i } \right\} _ { i } ,$ control group $\left\{ Y _ { i } \right\} _ { i }$ , effect size $= \frac { \overline { X } - \overline { Y } } { s _ { \text { pool } } }$  
...
|       | $H _ { A }$ | type II error (FN) | True Tositive: $S$     | $m - m _ { 0 }$ | 
| Total |             | $m - R$            |  $R$                   | $m$             |
...
$H _ { 0 } : \beta _ { 1 } = 0$; $H _ { \mathrm { A } } : \beta _ { 1 } \neq 0$ ; $T = \frac { \hat { \beta } _ { 1 } - 0 } { \widehat { \mathrm { se } } \left( \hat { \beta } _ { 1 } \right) }$ ; $H _ { 0 } : T \sim t _ { n - 2 }$ ; $K = \left( - \infty , - t _ { n - 2,1 - \frac { \alpha } { 2 } } \right] \cup \left[ t _ { n - 2,1 - \frac { \alpha } { 2 } } , \infty \right)$ , $| T | > t _ { n - 2,1 - \frac { \alpha } { 2 } } : reject$

#####  Coefficient of determination $R ^ { 2 } = \left( \frac { s _ { \hat { y } y } } { s _ { \hat { y } } s _ { y } } \right) ^ { 2 }$
$\hat { \beta } _ { 1 } = \frac { s _ { x y } } { s _ { x } ^ { 2 } } = \frac { \operatorname { Cov } ( X, Y ) } { \operatorname { Var } ( Y ) } = \operatorname { Cor } ( Y , X ) \frac { \operatorname { Sd } ( Y ) } { \operatorname { Sd } ( X ) }$ , $s _ { x y } = \frac { 1 } { n - 1 } \sum _ { i = 1 } ^ { n } \left( x _ { i } - \overline { x } \right) \left( y _ { i } - \overline { y } \right)$ ; $\hat { \beta } _ { 0 } = \overline { Y } - \hat { \beta } _ { 1 } \overline { X }$

##### Fitting linear model to transformed data: $\log \left( Y _ { i } \right) = \hat { \beta } _ { 0 } + \hat { \beta } _ { 1 } \log \left( X _ { i } \right) + E _ { i }$  
